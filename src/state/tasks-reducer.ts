...
export type ActionsTypes = AddTaskActionType | RemoveTaskActionType  | ChangeTaskStatusActionType | ChangeTaskTitleActionType
...
export const tasksReducer = (state: TasksStateType, action: ActionsTypes): TasksStateType => {
    switch (action.type) {
        ...
        case "CHANGE-TASK-STATUS": {
            return {
            ...state, [action.tdlID]: state[action.tdlID].map(el => el.taskID === action.taskID ? {...el, isDone: action.isDone} : el)
            }
        }
        case "CHANGE-TASK-TITLE": {
            return{
            ...state, [action.tdlID]: state[action.tdlID].map(el => el.taskID === action.taskID ? {...el, title: action.title} : el)
            }
        }
        default:
